...
if ($_POST && isset($_POST['action']) && $_POST['action'] === 'create_task') {
    ...
    if (!$stmt->fetch()) {
        ...
        try {
            ...
            $pdo->commit();
            $success = "Zadanie zostało utworzone z $url_count elementami i dodane do kolejki. Najpierw zostanie pobrana treść stron, a następnie wygenerowane opisy.";
...
// Pobierz zadania z poprawnym statusem - NAPRAWIONE ZAPYTANIE
...
$stmt = $pdo->prepare("
    SELECT t.*, p.name as project_name, ct.name as content_type_name,
           ...
           COUNT(DISTINCT CASE WHEN ti.status = 'completed' THEN ti.id END) as completed_count,
           COUNT(DISTINCT CASE WHEN ti.status = 'failed' THEN ti.id END) as failed_count,
           COUNT(DISTINCT CASE WHEN ti.status = 'processing' THEN ti.id END) as processing_count,
    ...
    GROUP BY t.id, t.project_id, t.content_type_id, t.name, t.status, t.strictness_level, t.task_data, t.created_at, p.name, ct.name
    ORDER BY t.created_at DESC
");
$stmt->execute($params);
$tasks = $stmt->fetchAll();

// Aktualizuj status zadań na podstawie statusów elementów
foreach ($tasks as &$task) {
    $total = $task['item_count'];
    $completed = $task['completed_count'];
    $failed = $task['failed_count'];
    $processing = $task['processing_count'];
    ...
    // Aktualizuj status w bazie danych jeśli się zmienił
    if ($task['status'] !== $new_status) {
        $stmt = $pdo->prepare("UPDATE tasks SET status = ? WHERE id = ?");
        $stmt->execute([$new_status, $task['id']]);
                                ...
                                <?php foreach ($tasks as $task): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php if ($task['failed_count'] > 0): ?>
                                                <small class="text-danger">(<?= $task['failed_count'] ?> błędów)</small>
                                            <?php endif; ?>
                                            <?php if ($task['item_count'] > 0): ?>
                                                <div class="progress mt-1" style="height: 5px;">
                                                    <div class="progress-bar bg-success" style="width: <?= ($task['completed_count'] / $task['item_count']) * 100 ?>%"></div>
                                                    <?php if ($task['failed_count'] > 0): ?>
                                                        <div class="progress-bar bg-danger" style="width: <?= ($task['failed_count'] / $task['item_count']) * 100 ?>%"></div>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                ...
                <form method="POST" id="taskForm">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info">
                            ...
                            <strong>Proces generowania:</strong> Po utworzeniu zadania system najpierw pobierze treść ze wskazanych stron internetowych, a następnie wygeneruje opisy kategorii na podstawie tej treści.
                        </div>
