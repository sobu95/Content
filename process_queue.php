...
function logMessage($message, $type = 'info') {
    ...
    // Zapisz do pliku log tylko ważne informacje
    if ($type !== 'debug' && !strpos($message, 'No items in queue') && !strpos($message, 'waiting...')) {
...
function processTaskItem($pdo, $queue_item, $api_key) {
    ...
    if (empty($task_item['page_content'])) {
        logMessage("Page content not available for task item $task_item_id, fetching now...");
        $page_content = fetchAndSavePageContent($pdo, $task_item_id, $task_item['url']);
        $task_item['page_content'] = $page_content;
    } else {
        logMessage("Using existing page content for task item $task_item_id");
    }
...
do {
    ...
    try {
        ...
        // Sprawdź czy elementy mają pobraną treść strony
        $stmt = $pdo->prepare("
            ...
            AND (ti.page_content IS NOT NULL AND ti.page_content != '')
